...
comp(function Account() {
  ...
  .template(
    (el) => html`
      ...
      <button
        @click=${() => accounts.setActive(el.id.value)}
        ?disabled=${el.id === el.active.id}
      >
        activate
      </button>
      <input value=${el.data.name} @change=${el.setName} />
      <br />
      <small class=${el.id === el.active.id ? "active" : ""}
        ><code>${el.id}</code></small
      >
      <br />
  ...
  .style(
    ".active { color: #090; font-weight: bold } img { margin-bottom: 2em }",
  );

comp(function AccountList() {
  return accounts;
}).template((el) => {
  const signed = el.active.sign(
    new TextEncoder().encode(`hello from ${el.active.data.name}`),
  );
